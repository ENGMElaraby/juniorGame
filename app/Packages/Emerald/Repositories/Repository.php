...
abstract class Repository
{
    ...
    public static function getInstance(): Repository
    {
        ...
        $parameter = (new \ReflectionClass($class))->getConstructor()->getParameters()[0]->getType()->getName();
...
//            call_user_func_array($method, $parameters);
     ...
     * @return $this
     */
    final public function where(string $column, ?string $value, string $operator = '='): self
    {
     ...
     * @return $this
     */
    final public function orWhere(string $column, ?string $value): self
    {
        $this->orWheres[] = compact('column', 'value');
     ...
     * @return $this
     */
    final public function whereIn(string $column, string|array $value): self
    {
        $this->whereIns[] = compact('column', 'value');
     ...
     * @return $this
     */
    final protected function setClauses(): self
    {
        ...
        foreach ($this->orWhere as $where) {
            $this->query->orWhere($where['column'], $where['value']);
        }

        foreach ($this->whereIns as $whereIn) {
            $this->query->whereIn($whereIn['column'], $whereIn['value']);
        }
