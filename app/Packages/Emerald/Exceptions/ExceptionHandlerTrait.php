...
//                \Log::channel('emerald')->info(json_encode($this->errors, $exception->getMessage()));
    ...
    {
        $theMessage = collect($this->errors)->first();
        if (isset($theMessage[0])) {
            $theMessage = $theMessage[0];
        }

        return response()->json(['status' => $statusCode, 'message' => $theMessage ?? $message, 'errors' => $this->errors], $statusCode, ['Access-Control-Allow-Origin' => '*'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
    }
